...
class AtivoCarteiraDao {
  ...
  Future<List<AtivoCarteira>> listarAtivosCarteira() async {
    ...
    var sql = "SELECT c.$COLUNA_ID, c.$COLUNA_ATIVO_ID, c.$COLUNA_QUANTIDADE, c.$COLUNA_PRECO_MEDIO, a.${AtivoDao.COLUNA_NOME}, a.${AtivoDao.COLUNA_TICKER}, a.${AtivoDao.COLUNA_TIPO} FROM $NOME_TABELA c INNER JOIN ${AtivoDao.NOME_TABELA} a"
        +" ON a.${AtivoDao.COLUNA_ID} = c.$COLUNA_ATIVO_ID ";
    final rows =    await _databaseHelper.query(sql);
    rows.forEach((linha) {
      var ativo = Ativo(
          linha[COLUNA_ATIVO_ID], linha[AtivoDao.COLUNA_TICKER], linha[AtivoDao.COLUNA_TIPO], linha[AtivoDao.COLUNA_NOME]);
